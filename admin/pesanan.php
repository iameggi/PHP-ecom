...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($mysqli,"select*from pesanan order by id desc");
			$j = mysqli_num_rows($q);
		?>
		...
		<?php
			if(!empty($_GET)){ 
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysql_query($mysqli,"select*from pesanan where id='$_GET[id]'"));
				?>
					...
					<form action="pesanan.php?act=edit&&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data">
						...
						<?php
							...
							<?php
								$katpro = mysqli_query($mysqli,"select*from user");
